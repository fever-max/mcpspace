...
import { resolveClaudeCodePaths } from './claude-code-paths.js'
...
export class ClaudeCodeAdapter implements ClientAdapter {
  ...
  constructor(
    ...
    const resolved = resolveClaudeCodePaths(cwd)
  ...
  async readCurrentState(): Promise<ActualState> {
    if (!this.detected) {
      return { clientName: 'claude-code', mcpServers: {} }
    }

    try {
      const raw = await readFile(this.configPath, 'utf-8')
      const parsed = JSON.parse(raw) as unknown
      return toActualState('claude-code', parsed)
    } catch (error) {
      if (isEnoent(error)) {
        return { clientName: 'claude-code', mcpServers: {} }
      }
  ...
  async applyPlan(plan: AdapterSyncPlan): Promise<void> {
    if (!this.detected) {
      throw new Error('Claude Code config not detected')
    }
  ...
  async restore(backupPath: string): Promise<void> {
    if (!this.configPath) {
      throw new Error('Claude Code config path is not resolved')
    }
  ...
  async validate(): Promise<ValidationResult> {
    if (!this.detected) {
      return { valid: false, errors: ['Claude Code config not detected'] }
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Invalid Claude Code config'
