...
export class CursorAdapter implements ClientAdapter {
  ...
  constructor(
    ...
    const resolved = resolveCursorPaths(cwd)
    this.configPath = resolved.projectConfigPath
  ...
  async readCurrentState(): Promise<ActualState> {
    if (!this.detected) {
      return { clientName: 'cursor', mcpServers: {} }
    }

    try {
      const raw = await readFile(this.configPath, 'utf-8')
      const parsed = JSON.parse(raw) as unknown
      return toActualState('cursor', parsed)
    } catch (error) {
      if (isEnoent(error)) {
        return { clientName: 'cursor', mcpServers: {} }
      }
  ...
  async applyPlan(plan: AdapterSyncPlan): Promise<void> {
    if (!this.detected) {
      throw new Error('Cursor config not detected')
    }
  ...
  async restore(backupPath: string): Promise<void> {
    if (!this.configPath) {
      throw new Error('Cursor config path is not resolved')
    }
  ...
  async validate(): Promise<ValidationResult> {
    if (!this.detected) {
      return { valid: false, errors: ['Cursor config not detected'] }
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Invalid Cursor config'
